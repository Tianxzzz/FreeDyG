...
class GraphAttentionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        # batch_size, num_neighbors, time_feat_dim =  node_time_features.shape
        # srcseq= node_time_features[:int(batch_size/2),:,:]
        # dstseq= node_time_features[int(batch_size/2):batch_size,:,:]
        # seq=torch.cat([srcseq,dstseq],dim=1)
        # low=int(np.around(200/3/2))
        # band=2*low
        # x = torch.fft.rfft(seq, n=200, dim=0, norm='forward')
        # x[low:,:]=0
        # sequence_emb_fft = torch.fft.irfft(x, n=200, dim=0, norm='forward')
        # node_time_features = sequence_emb_fft[0:int(batch_size/2),:].reshape(batch_size,num_neighbors,time_feat_dim)
        ...
        if current_layer_num == 0:
            ...
            # srcseq= node_time_features[:int(batch_size/2),:,:].unsqueeze(-1)
            # dstseq= node_time_features[int(batch_size/2):batch_size,:,:].unsqueeze(-1)
            # seq=torch.cat([srcseq,dstseq],dim=1)
            ...
            # x[low:,:]=0
            # sequence_emb_fft = torch.fft.irfft(x, n=200, dim=0, norm='forward')
            # neighbor_time_features = sequence_emb_fft[0:int(batch_size/2),:].reshape(batch_size,num_neighbors,time_feat_dim)
